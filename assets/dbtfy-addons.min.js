(function() {
  ...
  /* start-dbtfy-live-view */ function themeLiveView(){function LiveView(){const e=document.querySelectorAll(".dbtfy-live_view");if(!e.length)return;const t=sessionStorage.getItem("liveView"),i=theme.settings.dbtfyLiveViewMin,n=theme.settings.dbtfyLiveViewMax,s=theme.settings.dbtfyLiveViewIntervalSteps,o=theme.settings.dbtfyLiveViewIntervalTime;function r(t){e.forEach(e=>{const i=e.querySelector(".number-live_view");i&&(i.innerHTML=` ${t}`)})}t&&r(t),setInterval(function(){const e=+sessionStorage.getItem("liveView"),t=theme.getRandomNumberInRange(1,s),o=e||theme.getRandomNumberInRange(i,n),m=Math.random();let v=0,a=0;v=m<.5?(a=o-t)>i?a:i:(a=o+t)<n?a:n;sessionStorage.setItem("liveView",v),r(v)},o)}LiveView();} function themeLiveViewDestroy() {function destroyLiveView(){const e=document.querySelectorAll(".dbtfy-live_view");if(e.length&&e.forEach(e=>{e.remove()}),document.body.classList.contains("dbtfy-delivery_time-destroy")){document.querySelectorAll(".live-view-delivery-time-features").forEach(e=>{e.remove()})}localStorage.removeItem("liveView")}destroyLiveView();} /* end-dbtfy-live-view */
